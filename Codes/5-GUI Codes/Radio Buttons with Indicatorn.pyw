...
for txt,val in array:  #txt shows language and val shows its value in one index of array(list).
    ...
    Radiobutton(win,text=txt,variable=var,value=val,padx=20,pady=15,font=("Arial Black",18,'bold'),command=WelcomeWindow,indicatoron=1).pack(anchor=W)
    #This function is used to create a Radio button.Indicator is used for making the radio button option to be selected as whole text area instead of circle check.
